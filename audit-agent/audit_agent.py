...
class EmeraldTabletAgent:
    ...
    def run_tool(self, name, cmd):
        ...
        try:
            result = subprocess.run(
                cmd, shell=True, capture_output=True, text=True, timeout=300, cwd=str(self.target)
            )
    ...
    def layer1_static(self):
        ...
        self.run_tool("pylint", "pylint --recursive=y --output-format=text --score=no . 2>/dev/null || true")
        self.run_tool("flake8", "flake8 . 2>/dev/null || true")
        self.run_tool("shellcheck", "find . -name '*.sh' -exec shellcheck -f gcc {} + 2>/dev/null || true")
        self.run_tool("yamllint", "yamllint . 2>/dev/null || true")

    def layer2_security(self):
        print("\n=== LAYER 2: SECURITY ANALYSIS ===")
        self.run_tool("bandit", "bandit -r . -f txt -ll 2>/dev/null || true")
        self.run_tool("semgrep", "semgrep scan --config auto . --text 2>/dev/null || true")
        self.run_tool("gitleaks", "gitleaks detect --source . --verbose 2>/dev/null || true")

    def layer3_vulns(self):
        print("\n=== LAYER 3: VULNERABILITY SCAN ===")
        self.run_tool("trivy_fs", "trivy fs . --severity HIGH,CRITICAL 2>/dev/null || true")
        self.run_tool("trivy_config", "trivy config . 2>/dev/null || true")

    def layer4_types(self):
        print("\n=== LAYER 4: TYPE CHECKING ===")
        self.run_tool("mypy", "mypy . --ignore-missing-imports 2>/dev/null || true")
